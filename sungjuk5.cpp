#include <iostream>
#include <string>
//<iostream>과 <string>을 컴파일전에 소스에 확장하도록 지시
...
float ChangeGrade(string a)
{
	...
	else if ((a == "A0") || (a == "A"))	//성적이 A일때
	{
		return 4.0;
	}

	else if (a == "B+")	//성적이 B+일때
	{
		return 3.5;
	}

	else if ((a == "B0") || (a == "B"))	//성적이 B일때
	{
		return 3.0;
	}

	else if (a == "C+")	//성적이 C+일때
	{
		return 2.5;
	}

	else if ((a == "C0") || (a == "C"))	//성적이 C일때
	{
		return 2.0;
	}

	else if (a == "D+")	//성적이 D+일때
	{
		return 1.5;
	}

	else if ((a == "D0") || (a == "D"))	//성적이 D일때
	{
		return 1.0;
	}
...
{
	float FloatGrade = ChangeGrade(Sub.Grade);	//Grade를 실수로 변환해서 FloatGrade에 초기화
...
{
	string SearchName;	//검색할 이름 받는 string객체 SearchName 선언

	cout << "검색 할 학생 이름 : ";
	InputValue(SearchName);

	for (int i = 0; i < StudentNum; i++)
	{		//SearchName이 std[0~StudentNum ]중에 있을때
	...
	//SearchName이 std[0~StudentNum] 중에 없을때
	cout << "학생 이름을 다시 확인해 주세요.\n";
	return 0;	//0 반환

}

//과목 이름 검색 후 해당 과목이 있는지 확인
Subject* SubSearch(const Student* pSt)
{
	...
	{	//SEarchSub이 SubName중에 있을때
		if (SearchSub == pSt->Sub[i].SubName)
		{
			return (pSt->Sub) + i;	//해당 Sub을 반환
		}
	}

	//SearchSub이 SubName중에 없을때
	cout << "다시 입력해 주세요.\n";
	return 0;
}
...
void ModifyStdInfo(Student* pSt, int StudentNum)
{
	...
	while (1)
	{
		...
		else if (SelStr == "과목정보")
		{
			...
			cout << "* <" << tempModify->Sub->SubName << ", 학점 : " << tempModify->Sub->Hakjum << ", 등급 : " << tempModify->Sub->Grade << "> 의 정보를 수정하세요.\n";
			cout << "교과목명 : ";
