...
Student* StdSearch(Student* pSt, int StudentNum)
{
	...
	if (strcmp(SearchName, pSt[0].StdName) == 0)	//SearchName[10]이 std[0]일때
	{
		return pSt;
	}

	else if (strcmp(SearchName, pSt[1].StdName) == 0)	//SearchName[10]이 std[1]일때
	{
		return pSt + 1;
	}

	else	//SearchName[10]이 std[0~1]중에 없을때
	{
		return 0;
	}
}
...
int main(void)
{
	...
	while (true)	//4 입력전에 끝나지 않는 반복문
	{
		...
		else if (num == 3)	//학생 이름 검색
		{
			...
			Student* temp = StdSearch(stu, 2);	//구조체 배열 temp에 서치한 후 결과 저장

			if (temp != 0)	//검색 학생이 있을시
			{
				PrintOneData(*temp);
			}
